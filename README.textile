...
# batch.search takes the same arguments as Model.search.
batch.search 'foo'
batch.search :conditions => {:name => 'bar'}
batch.search 'baz', :with => {:active => true}

# When you call batch#searches, the searches are all populated:
...
* Timestamp MVAs
* Search option and/or configuration option to define class descendants

...
Currently Thinking Sphinx is built to support Rails/ActiveRecord 6.1 or newer. It's possible that Rails 5.0-6.0 will work, but it's no longer tested against. If you're using Sinatra and ActiveRecord instead of Rails, that's fine - just make sure you add the @:require => 'thinking_sphinx/sinatra'@ option when listing @thinking-sphinx@ in your Gemfile.

The last release to support Rails 4.2 is v5.6.0.
...
You'll need either the standard Ruby (v3.0 or newer) or JRuby (9.1 or newer). It's possible that Ruby 2.7 will work, but it's no longer tested against.

The last release to support Ruby 2.4-2.7 is v5.6.0.

...
There's no gem (prerelease or otherwise) at this point - so you'll need to use the git reference. Also, you'll need to specify the Mysql2 gem as well (this is not an inbuilt dependency because JRuby, when supported, will need something different):

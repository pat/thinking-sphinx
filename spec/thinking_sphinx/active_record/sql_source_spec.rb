...
describe ThinkingSphinx::ActiveRecord::SQLSource do
  ...
  before :each do
    ThinkingSphinx::Adapters::MySQLAdapter.stub!(:=== => true)
    ThinkingSphinx::ActiveRecord::Base.stub!(:adapter_for => adapter)
  end
  
  describe '#adapter' do
    it "returns a database adapter for the model" do
      ThinkingSphinx::ActiveRecord::Base.should_receive(:adapter_for).
        with(model).
        and_return(adapter)
  ...
  describe '#type' do
    it "is mysql when using the MySQL Adapter" do
      ThinkingSphinx::Adapters::MySQLAdapter.stub!(:=== => true)
      ThinkingSphinx::Adapters::PostgreSQLAdapter.stub!(:=== => false)
      
      source.type.should == 'mysql'
    end
    
    it "is pgsql when using the PostgreSQL Adapter" do
      ThinkingSphinx::Adapters::MySQLAdapter.stub!(:=== => false)
      ThinkingSphinx::Adapters::PostgreSQLAdapter.stub!(:=== => true)
